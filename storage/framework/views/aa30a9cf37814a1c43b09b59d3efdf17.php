...
<div id="chat-floating-btn" class="chat-floating-btn">
    <i class="fas fa-comments"></i>
</div>

<!-- ✅ Right-Side Floating Chatbox (Hidden by default) -->
<div id="chatbox" class="chatbox-wrapper d-none">
...
<script>
    // Safe fallback for labels and data
    const branchLabels = <?php echo json_encode($branchNames ?? [], 15, 512) ?>;
    const booksData = <?php echo json_encode($booksCount ?? [], 15, 512) ?>;
    const studentsData = <?php echo json_encode($studentsCount ?? [], 15, 512) ?>;
    const attendanceLabels = <?php echo json_encode($labels ?? [], 15, 512) ?>;
    const attendanceDatasets = <?php echo json_encode($datasets ?? [], 15, 512) ?>;

    // Books per Branch Line Chart
    if (branchLabels.length && booksData.length) {
        const ctxBooks = document.getElementById('booksBranchLineChart').getContext('2d');
        new Chart(ctxBooks, {
            type: 'line',
            data: {
                labels: branchLabels,
    ...
    if (branchLabels.length && studentsData.length) {
    ...
    new Chart(ctxAttendance, {
        type: 'line',
        data: {
            labels: attendanceLabels, // ✅ Now uses safe fallback
            datasets: attendanceDatasets // ✅ Also safe
        },
        options: {
            responsive: true,
            maintainAspectRatio: false,
            plugins: {
                title: {
                    display: true,
                    text: 'Monthly Attendance - <?php echo e(\Carbon\Carbon::now()->format("F Y")); ?>'
                }
            },
            scales: {
                y: { 
...
<?php $__env->stopPush(); ?>
<?php echo $__env->make('layouts.librarian', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH F:\laragon\www\library\resources\views/dashboard/librarian.blade.php ENDPATH**/ ?>